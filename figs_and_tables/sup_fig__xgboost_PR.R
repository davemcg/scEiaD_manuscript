...
# umapRef <- umap
pr_calculator <- function(predictions, partition){
  if (partition != 'All'){
    predictionsC <- predictions %>% 
  ...
  one_hot_labelled <- model.matrix(~0+as.factor(pr_df$TrueCellType)) %>% as.data.frame
  colnames(one_hot_labelled) <- strsplit(colnames(one_hot_labelled), ')', fixed = T ) %>% 
  ...
  pr_df <- bind_cols( one_hot_labelled, pr_df)
  ...
  # out <- list()
  # out$xgboost_pr_plot <- xgboost_pr
  # out$xgboost_pr_table <- xgboost_pr_table
  # out
...
for (i in c('All', umap$study_accession %>% unique())){
  #print(i)
  suppressWarnings({
    try({pr_list[[i]] <- pr_calculator(predictions, i)}, silent = TRUE)
    })
}
