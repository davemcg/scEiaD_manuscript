...
The  recent introduction of relatively low cost and high throughput single cell sequencing technlogy has led to an explosion of research across many fields. GIVE SOME EXAMPLES. As of 2020, over 30 million cells have been sequenced across n studies (Svennson Pachter http://www.nxn.se/single-cell-studies)

## Many in the eye 
  Some of the seminal work in the high throughput single cell transcriptomics field (Macosko et al) have used the retina. As of late 2020, over a dozen published studies have used single cell technology to profile cell type specific gene expression patterns, developmental trends, species specific differentiation, and disease alterations. 

## Impossible to easily compare results across independent studies
...
# scPOP wraps several different methods at measuring integration performance. 
  ...
  Local Simpson Index (LISI) and Average Silhouette Width (ASW) metrics from Harmony and kBET, respectively, are used as well as the Adjusted Rand Index (ARI), Normalized Mutual Information (NMI), and PCR metrics as implemented by scIB. The LISI and ASW were used to measure batch mixing (where lower is better), celltype mixing (higher is better), and cluster mixing (higher and better). NMI and ARI were used to assess the consistency of celltype to cluster assignment (where 1 is perfect correspondence between cluster and cell type). The PCR metric (higher is better) runs PCA before and after integration to assess how much variance is explained by the batch corrected dimensional space. 
...
## scVI has highest integration performance
  ...
  To rank the integration algorithms across the various benchmarks, scPOP Z scale normalizes all the metrics so that each score has an equal weight. scPOP then sums them all which allows us to rank the performance of each integration tool across the benchmarks. For users who have custom needs, the weights of each metrics can be customized to prioritize mixing or sepraration. scPOP finds that scVI has the strongest performance, with a sum score of SOMETHING. The next highest performers are Harmony, fastMNN, and Scanorama. We do not claim that these results are gegeral - rather we emphasize that for each batch correction analysis that scPOP or some similar approach be used to identify the optimal method for each dataset. 
