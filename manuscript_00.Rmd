...
load('~/data/massive_integrated_eye_scRNA/n_features-5000__transform-counts__partition-universe__covariate-batch__method-scVIprojectionSO__dims-8__preFilter.scEiaDprojected__dist-0.1__neighbors-500.umap.Rdata')
umapRef <- umap
...
load('~/data/massive_integrated_eye_scRNA/PREDICTIONSn_features-5000__transform-counts__partition-universe__covariate-batch__method-scVIprojectionSO__dims-8__preFilter.scEiaDprojected__dist-0.1__neighbors-500.Rdata')
test_predictions <- data.table::fread('~/data/massive_integrated_eye_scRNA/277874_CTpredictortest_data_probabilities.csv.gz') %>% as_tibble %>% 
...
# pmid search
load('~/data/massive_integrated_eye_scRNA/top_marker_pmid.Rdata')
```
...
<!-- # REWRITE? (UNFORTUNATELY) FOR NEW DATA WHEN(?) IT COMES IN -->
As another check on the quality of the cell type assignments, we ran pairwise differential expression tests between the predicted cell types (see methods for further details). We show the five most consistently differentially expressed genes for each of the major retina cell types (`r supFig_cap(name = 'figXGboost_umap', display = 'cite')`a). As a simple metric to identify known and unknown genes relating to the cell type specific expression we search PubMed for the number of publications with two searches per gene. The first search is the more precise "gene AND cell type" (e.g. "PDE6H AND Cones") and the second search is the more inclusive "gene AND retina" (e.g. "PDE6G AND Retina"). Of the 55 genes in (`r supFig_cap(name = 'figXGboost_umap', display = 'cite')`a), 37 had one or more citations in the gene/celltype search (table X) and 47 had one or more citation in the inclusive search. The 55 genes had a mean of `r top_markers %>% group_by(cluster)  %>% slice_max(med_auc, n =5) %>% pull(c2) %>% mean() %>% round()` studies (with the inclusive gene by "retina" search). In contrast, 100 randomly chosen genes had a mean of `r pmid3 %>% map(function(x) sum(!is.na(x))) %>% unlist() %>% enframe() %>% pull(value) %>% mean() %>% round`. 

The four genes with zero hits were IGSF21 (Bipolar Cells), BTBD8 (Bipolar Cells), FRZB (Muller Glia), and TAGLN3 (Retinal Ganglion Cells). In a more exhaustive hand literature search we find that IGSF21 is associated with Diabetic Retinopathy in a Han Chinese population (https://pubmed.ncbi.nlm.nih.gov/27607899/). BTBD8 contains BTB/POZ domains associated with transcription factor protein-protein interactions (https://www.spandidos-publications.com/10.3892/ijmm.13.1.193). FRZB was previously referred to as SFRP3 and this Wnt pathway member has been previously shown to be active in Muller Glia (https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2330018/). TAGLN3 has been previously reported in chick to be expressed in developing Retinal Ganglion Cells (https://onlinelibrary.wiley.com/doi/abs/10.1002/cne.24241). 
<!-- # REWRITE? (UNFORTUNATELY) FOR NEW DATA WHEN(?) IT COMES IN -->
...
scran findmarkers (wilcox test) along with singleCellHaystack. Filtering to keep genes that FDR < 1 or

## Trajectory

To calculate continuous trajectories for individual terminally differentiated retina cell types, we used the Slingshot R package [@streetSlingshotCellLineage2018]. Slingshot first uses the clusters to build a minimum spanning tree to identify the global lineage structure. We specified the starting cluster by identifying the cluster for human and mouse with a high proportion of the early RPC cells and specified the terminal clusters that were predominantly a terminal retina cell type (Amacrine, Bipolar, Cones, Horizontal, Muller Glia, Retinal Ganglion, Rods). XXXX trajectories were built and we hand-pruned out redundant or non-sensical (e.g. Photoreceptor precursors to Cone to Rod Bipolar) trajectories, leaving XXXX trajectories for downstream analysis. 
