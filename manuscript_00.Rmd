...
As we noticed that several retina marker genes (WHICH ONES!!!!) had unusually low expression in the macaque data we quantified the scRNA data twice: once with the Ensembl reference and again with the same Gencode human reference used for the human data. We compared the gene-level counts for each cell and replaced the macaque gene count with the human counts if the human counts were higher. VINAY AM I MISSING DETAILS?
...
We wrote the scPOP R package to unify the LISI and Silhouette metrics from Harmony and kBet, respectively, along with NMI and ARI. LISI and Silhouette require a dense matrix, which is a problem for our data as a 1e6 x 20,000 matrix cannot fit even in a 1.5 TB memory node. We down-sampled the dataset to ~100,000 cells, taking care to keep rarer cell types for the LISI and Silhouette benchmarking.
