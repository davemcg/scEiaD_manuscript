...
```{r figBenchmark, fig.width=6, fig.height=4, fig.cap=figBenchmark_cap, echo=FALSE, message = FALSE}
figBenchmark_cap <- fig_cap(name='figBenchmark', caption =  'a. Example of a method (Combat) which has a high level of batch blending, but poor separation of cell types (colored by cell type). b. example of an approach (no batch correction) that cleanly separates cell types but does not mix batches (colored by study). d. Optimal parameters for scVI method. sumZScale not directly to c')
...
supFig2_cap <- supFig_cap(name='supFig2_cap', caption =  'Performance of the various batch correction tools across various benchmarking metrics. For the LISI and Silhouette plots in A, B higher means better batch mixing and further to the right means better cluster purity. For the ARI and NMI metrics (which reflects how well cluster matches with cell type) in C, D, higher means a better score.')
