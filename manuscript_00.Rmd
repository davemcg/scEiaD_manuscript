...
## Projection of new data onto scEiaD shows that outside datasets for both tissue and iPSC-based samples overlap the scEiaD/scVI model
...
Projection of new data onto scEiaD allows for semi-direct comparison of other species, *in vitro* systems, and pharamacologic / mutational perturbation of mammalian retina cells. In the previous example we demonstrated how a outside mouse pan-retina dataset maps onto 

We demonstrate with a iPSC-based RPE cell line that we ran with the 10x v2 system that non-primary cells can be projected onto the scEiaD. The differentiated iPSC-based RPE from this highly optimized differentiation protocol (CITE BHARTI) overlap post-mortem adult human RPE cells from the Cowan et al. work. (MAKE FIGURE)
